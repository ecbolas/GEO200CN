...
cor(studentscores$group, studentscores$mean) #this returns a different number...

m <- lm(age ~ mean, data = studentscores)
m
cor(studentscores$age, studentscores$mean)#see above

plot(x = studentscores$group, y = studentscores$mean, pch = 21, bg="blue")

#abline(d, col = "red", lwd = 2) doesn't work

plot(x = studentscores$age, y = studentscores$mean, pch = 21, bg="blue")
#abline(m, col= "red", lwd = 2) this doesn't seem to work
abline(a= 20.2908, b=0.2159) #not sure about how to add in this line
abline(lm(age ~ mean, data = studentscores), bg = "red")
